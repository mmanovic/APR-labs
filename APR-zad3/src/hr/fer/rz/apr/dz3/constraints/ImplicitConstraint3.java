...
public class ImplicitConstraint3 implements IConstraint {
	@Override
	public boolean satisfy(Matrix point) {
		return 3 - point.get(0, 1) - point.get(0, 0) >= 0;
	}

	@Override
	public double constraintValue(Matrix point, double t) {
		double tmp = 3 - point.get(0, 1) - point.get(0, 0);
		if (tmp <= 0) {
	...
	public double constraintValue(Matrix point) {
		return 3 - point.get(0, 1) - point.get(0, 0);
	}
